...
<?php $tongtaiche = $taiche['taiche_soluong']*$taiche['taiche_dongia']; 
if($tongtaiche !== 0){
	$giatridonhang = $giatridonhang - $tongtaiche;
}
?>

<div style="margin-top: 5px; margin-bottom: 2px;"></div>
<?php if(count($cart) > 0){ ?>
<table class="sales_table_100 border-line-top" id="taiche">
	<colgroup>
		<col width="40%"><col width="8%"><col width="2%"><col width="5%"><col width="15%"><col width="2%"><col width="20%">
	</colgroup>
	<tbody>
		<tr>
			<?php echo form_open($controller_name."/update_taiche", array('class'=>'form-horizontal', 'id'=>'form_taiche')); ?>
			<td>Chi phí tái chế</td>
			<td><?php echo form_input(array('id' => 'taiche_soluong','style'=>'','placeholder'=>'Số lượng','name'=>'taiche_soluong','size'=>'2', 'class'=>'form-control input-sm', 'value'=>$taiche['taiche_soluong'], 'tabindex'=>++$tabindex));?></td>
			<td>kg</td><td></td>
			<td><?php echo form_input(array('id' => 'taiche_dongia','style'=>' margin-right: 9px;','placeholder'=>'Đơn giá','name'=>'taiche_dongia','size'=>'2', 'class'=>'form-control input-sm', 'value'=>$taiche['taiche_dongia'], 'tabindex'=>++$tabindex));?></td><td>Đ</td>
			<?php echo form_close(); ?>
			<td ><?php echo "Thành tiền: ".to_currency($tongtaiche); ?></td>
		</tr>
	</tbody>
</table>
<?php } ?>
